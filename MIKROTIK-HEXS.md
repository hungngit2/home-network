...
| Service | State |
|---|---|
| SSH | **enabled** (Ed25519 key-based auth `yes-if-no-key` with `id_hungnguyen`, `strong-crypto=yes`, restricted to `10.0.0.0/24` + `10.0.100.0/24`, aliased in `~/.ssh/config` as `home` / `hexs`) |
