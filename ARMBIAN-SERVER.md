...
export DDNS_DOMAIN="example.ddns.net"    # DDNS / Host domain for TLS & redirection
export APPSRV_DIR="/mnt/appsrv"          # App data mount path
export NASDATA_DIR="/mnt/nasdata"        # Bulk storage mount path
export MYTV_AUTH_USER="admin"            # IPTV web auth username
export MYTV_AUTH_PASS="admin"            # IPTV web auth password
...
| `wifi-config-tool/` | OpenWrt WiFi Configuration Tool (`OpenWrt\DeviceManager`) — manages SSID, encryption, and 802.11r/k/v fast roaming across the 4 home APs via **direct SSH keys (`id_hungnguyen`)** (no passwords stored in `config.json`), protected with optional HTTP Basic Auth (`auth.php`). |
...
Other loose files at the web root: `c.m3u` / `r.m3u` (IPTV playlists), `unbound.sh` (an **OpenWrt** Unbound installer/config script for the router — not used on this Armbian box itself; likely staged here for distribution to the OpenWrt boxes in `wifi-config-tool/config.json`).
...
* **Headless SSH Batching (`OpenWrt\OpenWrtClient`)**: Communicates with OpenWrt APs via native SSH keys (`/var/www/.ssh/id_hungnguyen`), bypassing legacy LuCI JSON-RPC dependencies and eliminating plaintext passwords. Multiple UCI changes are executed in a single atomic batch (`uci set ... && uci commit && /sbin/wifi reload`), executing full AP reconfiguration in <350ms.
