...
| `wifi-config-tool/` | OpenWrt WiFi Configuration Tool (`OpenWrt\DeviceManager`) — manages SSID, encryption, and 802.11r/k/v fast roaming across the 4 home APs via **direct Ed25519 SSH keys (`id_hungnguyen`)** (no passwords stored in `config.json`), protected with optional HTTP Basic Auth (`auth.php`). Linked from `/wifi` on the web root for convenience. |
