...
Chainedbox is the only server dual-homed onto two of these (LAN + IoT) — everything else reaches it by routing through the MikroTik. Guest never touches Chainedbox at all, which is also why `avahi`'s mDNS reflection stops at LAN↔IoT and never extends to Guest.
...
- **[MIKROTIK-HEXS.md](MIKROTIK-HEXS.md)** — router config: VLANs, dual-WAN policy routing, VPN, firewall, DNS, IGMP/multicast, scheduler scripts.
- **[ARMBIAN-SERVER.md](ARMBIAN-SERVER.md)** — full service inventory for Chainedbox: storage layout, every service, network/VLAN wiring.
- **[WIFI-APS.md](WIFI-APS.md)** — the 4-AP fleet: VLAN/SSID scheme, mesh backhaul, roaming.
- **[configs/chainedbox/](configs/chainedbox/)** — the actual raw config files for Chainedbox (nginx vhost, systemd units, `aria2.conf`, `smb.conf`, `AdGuardHome.yaml`, `avahi-daemon.conf`, Unbound conf.d, `docker/daemon.json`, netplan, crontab, etc.), for a future reinstall/migration. The router and APs aren't archived this way since both have their own built-in config-export feature (`/export` on RouterOS, `sysupgrade` backup on OpenWrt).
