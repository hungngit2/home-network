#!/usr/bin/env bash
# ==============================================================================
# Debian / Armbian Home Server Automated Bootstrap & Installation Script
# https://github.com/hungngit2/home-network
# ==============================================================================
# Compatible with Debian 12 (Bookworm), Debian 13 (Trixie), Ubuntu 24.04, and Armbian
#
# Usage:
...
MYTV_AUTH_USER="${MYTV_AUTH_USER:-admin}"
MYTV_AUTH_PASS="${MYTV_AUTH_PASS:-admin}"
RTP2HTTPD_PORT="${RTP2HTTPD_PORT:-5140}"
UNBOUND_PORT="${UNBOUND_PORT:-5335}"
SMB_NETBIOS_NAME="${SMB_NETBIOS_NAME:-${DETECTED_HOSTNAME}}"
...
echo "  ╚═════╝ ╚══════╝╚═════╝ ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝"
echo -e "${NC}"
echo -e "${BOLD}Debian / Armbian Home Server Automated Bootstrap Installer${NC}\n"

if [[ "${NON_INTERACTIVE}" != "true" ]]; then
    echo -e "${YELLOW}Please review or adjust any configuration settings below (press Enter to accept default):${NC}\n"
    prompt_val "Primary Network Interface" "IFACE_NAME"
...
ACTION=="add|change", SUBSYSTEM=="block", ENV{ID_FS_LABEL}=="nasdata", RUN+="/usr/bin/systemctl --no-block restart mnt-nasdata.mount"
EOF
...
if [[ ! -f /opt/AdGuardHome/AdGuardHome ]]; then
    log_info "Downloading official AdGuard Home binary for ARM64 / x86_64..."
    ARCH="arm64"
    case $(uname -m) in
        x86_64) ARCH="amd64" ;;
...
log_info "Configuring Avahi mDNS reflector..."
fetch_repo_file "configs/chainedbox/avahi/avahi-daemon.conf" "/etc/avahi/avahi-daemon.conf"
if [[ "${IS_WIRELESS}" == "true" ]]; then
    sed -i "s/allow-interfaces=end0,end0.10/allow-interfaces=${IFACE_NAME}/g" /etc/avahi/avahi-daemon.conf
else
...
# Hardware acceleration: enable Intel QuickSync (QSV) / VAAPI if /dev/dri/renderD128 is available
if [[ -e /dev/dri/renderD128 ]]; then
    log_info "Intel GPU detected — installing iHD VA-API driver and enabling Jellyfin hardware acceleration..."
    apt-get install -y --no-install-recommends intel-media-va-driver-non-free mesa-opencl-icd clinfo vainfo libva-drm2 2>/dev/null || \
    apt-get install -y --no-install-recommends intel-media-va-driver mesa-opencl-icd clinfo vainfo libva-drm2 2>/dev/null || \
    ...
    # Deploy encoding.xml to enable QSV/VAAPI (only if not already customised by user)
    ...
    log_succ "Jellyfin hardware acceleration (QSV / VAAPI) enabled."
else
